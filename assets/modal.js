...
async function removeProject(id) {
	const errorMessage = document.querySelector('.remove-error-message')

	const response = await fetch('http://localhost:5678/api/works/' + id, {
		method: 'DELETE', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + localStorage.getItem('authToken') },
	})
...
async function handleSavePhoto() {
	...
	const errorMessage = document.querySelector('.add-error-message')
	...
	try {
		const response = await fetch('http://localhost:5678/api/works', {
			method: 'POST', headers: { 'Authorization': 'Bearer ' + localStorage.getItem('authToken') }, body: formData,
